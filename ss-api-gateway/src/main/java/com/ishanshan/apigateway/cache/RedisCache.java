package com.ishanshan.apigateway.cache;

import com.ishanshan.apigateway.filter.auth.LoginSession;
import com.ishanshan.apigateway.filter.auth.SystemInfo;
...
public class RedisCache {
    ...
    public LoginSession userSession(String eurekaServerName, String username) {
    ...
    @Cacheable(
        value = "systemCache",
        key =
                "T(String).valueOf(#eurekaServerName) + T(com.ishanshan.apigateway.cache.RedisKeyType).SYSTEM_INFO.key+ T(String).valueOf(#currentShopId)",
        sync = true
    )
    public SystemInfo systemInfo(String eurekaServerName, String currentShopId) {
        // TODO: 2019/1/9 通过Feigh获取并放入缓存
        //        SystemInfo systemInfo = authService.systemInfo(currentShopId);
