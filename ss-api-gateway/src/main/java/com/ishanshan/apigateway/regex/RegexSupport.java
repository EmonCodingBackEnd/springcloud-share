 ...
 ********************************************************************************/
package com.ishanshan.apigateway.regex;

import com.ishanshan.apigateway.regex.result.*;
...
public abstract class RegexSupport {
     ...
     * @since 1.0.0
     */
    public static GatewayUrlRegexResult matchGatewayUrl(String value) {
        GatewayUrlRegexResult result = GatewayUrlRegexResult.instance();
        Matcher matcher = RegexDefine.GATEWAY_URL_REGEX_PATTERN.matcher(value);
        if (matcher.matches()) {
            result.setMatched(true);
            result.setRegex(RegexDefine.GATEWAY_URL_REGEX);
            result.setPattern(RegexDefine.GATEWAY_URL_REGEX_PATTERN);
            result.setMatcher(matcher);
            result.setUrl(matcher.group(0));
